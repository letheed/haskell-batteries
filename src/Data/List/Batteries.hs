...
maximumIndices =
  map fst . head . groupBy ((==) `on` snd) . sortBy (flip (comparing snd)) . zip [0 ..]
...
groupOn f =
  let memo x = let y = f x in y `seq` (y, x) in map (map snd) . groupBy ((==) `on` fst) . map memo

splitOn :: (Eq a) => a -> [a] -> [[a]]
splitOn _ []     = []
splitOn c phrase = word : splitOn c (drop 1 rest) where (word, rest) = break (c ==) phrase

dedup :: (Ord a) => [a] -> [a]
dedup = map head . group . sort

elemOrd :: (Ord a) => a -> [a] -> Bool
elemOrd e lst | null ge   = False
              | otherwise = e == head ge
  where ge = dropWhile (e >) lst

maximumOn :: (Ord b) => (a -> b) -> [a] -> a
maximumOn f =
  let memo x = let y = f x in y `seq` (y, x) in snd . maximumBy (comparing fst) . map memo

sortGroupOn :: (Ord b) => (a -> b) -> [a] -> [[a]]
sortGroupOn f =
  let memo x = let y = f x in y `seq` (y, x)
